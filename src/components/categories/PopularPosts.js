          ...
          { popularPosts.map(({node:post}) => (
            <div key={post.id} className="column">
              <Link to={post.path} className="popular-post-card" >
                <Img className="popular-post-card-image" fluid={(post.featured_media && post.featured_media.localFile && post.featured_media.localFile.childImageSharp && post.featured_media.localFile.childImageSharp.fluid) || coverImg} alt={(post.featured_media && post.featured_media.alt_text) || 'post image'}  />
