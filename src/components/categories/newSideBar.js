...
const SideBar = ({ subcategories, noSubcategory }) => {
  ...
  return (
    <div className="column is-one-quarter-widescreen side-bar">
      ...
      <div
        className={`category-filters search-filters ${menu ? "is-active" : ""}`}
      >
        {noSubcategory ? null : subcategories.length > 0 ? (
          <>
            <h3>Sub categories</h3>
            <div className="subcategories-wrapper">
              {subcategories.map(({ node: category }, i) => (
                <button
                  className={`subcategories-btn ${category.name
                    .replace(/ .*/, "")
                    .toLowerCase()}`}
                  key={i}
                >
