...
const ThankYouMsg = () => (
  <span className="thank-you-msg">
    Thanks for helping us make PetPlace the ultimate destination for pet
    enthusiasts!
  </span>
)
...
const LikeArticleContent = (
  ...
  return successMsg ? (
    ...
    <>
      ...
      <span
        className="yes-btn"
        onClick={() => HandleSubmit(setSuccessMsg, url, wordpress_id, helpful)}
      >
        Yes
      </span>
      <span className="yes-btn" onClick={() => setFormState(true)}>
        No
      </span>
    </>
  )
