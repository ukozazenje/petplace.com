...
import desktopArrow from "../images/arrow-breadcrumbs.svg"
export default ({ category, desktop }) => {
  const { name, path } = category
  const categories = path
    .split("/")
    .filter(cat => cat && cat !== "article" && cat !== "category")
      ...
      {categories.map((cat, i) => {
        ...
        return (
          <Fragment key={i}>
            <Link
              to={categoryPath}
              className={`category-link ${categoryColor(name)}`}
            >
              {cat.replace(/-/g, " ")}
            </Link>
            {i < categories.length - 1 ? (
              <img
                className="divider"
                src={desktop ? desktopArrow : rightArrow}
                alt="right arrow"
              />
            ) : null}
          </Fragment>
