...
const Breeds = ({ data }) => {
  ...
  const allBreeds = [...data.allWordpressBreedPosts.edges]
  const limit = 2
  ...
  //   );
  // }
  // type: ['Sporting', 'Hound', 'Terrier', 'Toy', 'Non-Sporting', 'Herding'],
  ...
  useEffect(() => {
    if (isInitialMount.current) {
     ...
     setDisplayBreeds(breeds.filter( ({node:breed}) => {
       console.log('Tyep---->',breed.acf.type )
       console.log(types.includes( breed.acf.type ))
            ...
            <div className={`category-filters search-filters ${menu ? 'is-active' : ''}`}>
              ...
              <div className={`filter-type ${ filterType ? "is-active" : ""} `}>
                <div className="checkbox-wrapper">
                  <input type="checkbox" onChange={ (e) => handleChange(e.target.name) } checked={types.includes("Sporting")} name="Sporting" />
                  <span>Sporting</span>
                </div>
                <div className="checkbox-wrapper">
                  <input type="checkbox" onChange={ (e) => handleChange(e.target.name) } checked={types.includes("Hound")} name="Hound" />
                  <span>Hound</span>
                </div>
                <div className="checkbox-wrapper">
                  <input type="checkbox" onChange={ (e) => handleChange(e.target.name) } checked={types.includes("Terrier")} name="Terrier" />
                  <span>Terrier</span>
                </div>
                <div className="checkbox-wrapper">
                  <input type="checkbox" onChange={ (e) => handleChange(e.target.name) } checked={types.includes("Toy")} name="Toy" />
                  <span>Toy</span>
                </div>
                <div className="checkbox-wrapper">
                  <input type="checkbox" onChange={ (e) => handleChange(e.target.name) } checked={types.includes("Non-Sporting")} name="Non-Sporting" />
                  <span>Non-Sporting</span>
                </div>
                <div className="checkbox-wrapper">
                  <input type="checkbox" onChange={ (e) => handleChange(e.target.name) } checked={types.includes("Herding")} name="Herding" />
                  <span>Herding</span>
                </div>
              </div>
            </div>
